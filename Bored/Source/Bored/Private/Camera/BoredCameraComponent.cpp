...
UBoredCameraComponent::UBoredCameraComponent(const FObjectInitializer& ObjectInitializer) :Super(ObjectInitializer)
{
	ZoomAlpha = 1.0f;
...
void UBoredCameraComponent::OnZoomIn()
{
	SetZoomLevel(ZoomAlpha - 0.1f);
}

void UBoredCameraComponent::OnZoomOut()
{
	SetZoomLevel(ZoomAlpha + 0.1f);
}

void UBoredCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& OutResult)
{
...
void UBoredCameraComponent::UpdateCameraMovement(const APlayerController* InPlayerController)
{
	...
	if (LocalPlayer && LocalPlayer->ViewportClient && LocalPlayer->ViewportClient->Viewport)
	{
		...
		if (GetPlayerController() != NULL)
		{
			...
			{
				SpectatorCameraSpeed = GetDefault<UBoredSpectatorPawnMovement>(SpectatorPawn->GetMovementComponent()->GetClass())->MaxSpeed;
			}
		}
		if (!bNoScrollZone)
		{
...
void UBoredCameraComponent::MoveForward(float Val)
{
...
void UBoredCameraComponent::MoveRight(float Val)
{
	APawn* OwnerPawn = GetOwnerPawn();
	if (OwnerPawn != NULL)
	{
...
void UBoredCameraComponent::AddNoScrollZone(FBox InCoords)
{
	NoScrollZones.AddUnique(InCoords);
}

void UBoredCameraComponent::ClampCameraLocation(const APlayerController* InPlayerController, FVector& OutCameraLocation)
{
...
void UBoredCameraComponent::UpdateCameraBounds(const APlayerController* InPlayerController)
{
	ULocalPlayer* const LocalPlayer = Cast<ULocalPlayer>(InPlayerController->Player);
		...
		if (MyGameState)
		{
			...
			if (WorldBounds.GetSize() != FVector::ZeroVector)
			{
				...
				const FVector CameraPlanePoints[3] = {
					FBoredHelpers::IntersectRayWithPlane(WorldBoundPoints[0], FrustumRays[0], CameraPlane)*MiniMapBoundsLimit,
					FBoredHelpers::IntersectRayWithPlane(WorldBoundPoints[1], FrustumRays[1], CameraPlane)*MiniMapBoundsLimit,
					FBoredHelpers::IntersectRayWithPlane(WorldBoundPoints[2], FrustumRays[2], CameraPlane)*MiniMapBoundsLimit
				};
...
APlayerController* UBoredCameraComponent::GetPlayerController()
{
...
APawn* UBoredCameraComponent::GetOwnerPawn()
{
	return Cast<APawn>(GetOwner());
}

void UBoredCameraComponent::OnPinchStarted(const FVector2D& AnchorPosition1, const FVector2D& AnchorPosition2, float DownTime)
{
...
void UBoredCameraComponent::OnPinchUpdate(UBoredInput* InputHandler, const FVector2D& ScreenPosition1, const FVector2D& ScreenPosition2, float DownTime)
{
...
void UBoredCameraComponent::SetCameraTarget(const FVector& CameraTarget)
{
...
void UBoredCameraComponent::SetZoomLevel(float NewLevel)
{
	ZoomAlpha = FMath::Clamp(NewLevel, MinZoomLevel, MaxZoomLevel);
}

bool UBoredCameraComponent::OnSwipeStarted(const FVector2D& SwipePosition)
{
	bool bResult = false;
...
bool UBoredCameraComponent::OnSwipeUpdate(const FVector2D& SwipePosition)
{
	bool bResult = false;
	APlayerController* Controller = GetPlayerController();
...
bool UBoredCameraComponent::OnSwipeReleased(const FVector2D& SwipePosition)
{
	bool bResult = false;
...
void UBoredCameraComponent::EndSwipeNow()
{
	StartSwipeCoords.Set(0.0f, 0.0f, 0.0f);
}

bool UBoredCameraComponent::AreCoordsInNoScrollZone(const FVector2D& SwipePosition)
{
	bool bResult = false;
