...
bool FBoredHelpers::DeprojectScreenToWorld(const FVector2D& ScreenPosition, ULocalPlayer* Player, FVector& RayOrigin, FVector& RayDirection)
{
...
FVector FBoredHelpers::IntersectRayWithPlane(const FVector& RayOrigin, const FVector& RayDirection, const FPlane& Plane)
{
...
TSharedPtr<TArray<uint8>> FBoredHelpers::CreateAlphaMapFromTexture(UTexture2D* Texture)
{
...
FCanvasUVTri FBoredHelpers::CreateCanvasTri(FVector2D V0, FVector2D V1, FVector2D V2)
{
	FCanvasUVTri OutTri;
	OutTri.V0_Pos = V0;
	OutTri.V1_Pos = V1;
